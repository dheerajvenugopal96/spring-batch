...
public class SampleJob {
	...
	public Job secondJob() {
		return jobBuilderFactory.get("Second Job").incrementer(new RunIdIncrementer())
				.start(firstChunkStep()).next(secondStep()).build();
	}

	private Step firstChunkStep() {
		return stepBuilderFactory.get("first chunk step").<Integer, Long>chunk(4).reader(firstItemReader)
	...
	private Tasklet firstTask() {
		return new Tasklet() {
			...
			public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
				...
				System.out.println("inside tasklet " + chunkContext.getStepContext().getStepExecutionContext());
